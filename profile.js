...
function renderSheetTable(rows) {
    ...
    for (let r = 1; r < rows.length; r++) {
        ...
        for (let c = 0; c < rows[r].length; c++) {
            ...
            html += `<td style="padding: 6px 8px; border: 1px solid var(--border-color);"><input type="text" data-row="${r}" data-col="${c}" value="${escapeHtml(cellValue)}" style="width: 100%; background: var(--input-bg); color: var(--text-primary); border: none; padding: 6px 4px; box-sizing: border-box;"></td>`;
        }
        html += `<td style="padding: 6px 8px; border: 1px solid var(--border-color); text-align: center;"><button class="save-row-btn" data-row="${r}" style="background: var(--button-primary); border: none; border-radius: 20px; padding: 4px 10px; color: white; cursor: pointer;">${currentLang === 'ar' ? '💾 حفظ' : '💾 Save'}</button></td>`;
        html += '</tr>';
    }
    html += '</tbody>';
    html += '</table>';
